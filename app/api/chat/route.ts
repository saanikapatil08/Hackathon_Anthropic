...
import healthCategories from "@/app/lib/health_categories.json";
...
export async function POST(req: Request) {
  ...
  const categoryListString = healthCategories.categories
    .map((c) => c.id)
    .join(", ");

  const categoriesContext = USE_CATEGORIES
    ? `
    To help with our internal classification of health inquiries, we would like you to categorize student questions in addition to answering them. We have provided you with ${healthCategories.categories.length} health resource categories.
  ...
  const systemPrompt = `You are a helpful UMD Health Assistant chatbot designed to help University of Maryland students find health and wellness resources on campus. You provide information about mental health services, medical care, counseling, wellness programs, and other health-related resources available to UMD students. When responding to students, aim to provide clear, helpful, and compassionate responses while maintaining a supportive and professional tone.

  To help you answer the student's question, we have retrieved the following information from our UMD health resources database:
  ${isRagWorking ? `${retrievedContext}` : "No specific information found for this query, but I can still help with general guidance."}

  Please provide responses based primarily on the information you have been given. If the information is not sufficient or if the student needs immediate medical attention, advise them to contact the appropriate emergency service or health center directly.

  ${categoriesContext}

  If the question is unrelated to UMD health services or requires immediate medical attention, you should direct the student to appropriate resources or emergency services.
